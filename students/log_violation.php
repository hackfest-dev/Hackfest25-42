...
if (!isset($data['exam_id']) || !isset($data['violation_type'])) {
...
$sql = "SELECT COUNT(*) as count FROM cheat_violations 
        WHERE student_username = ? AND exam_id = ? AND violation_type = ?";
$stmt = $conn->prepare($sql);
$stmt->bind_param("sis", $student_username, $exam_id, $violation_type);
$stmt->execute();
...
$sql = "INSERT INTO cheat_violations (student_username, exam_id, violation_type, occurrence, penalty) 
        VALUES (?, ?, ?, ?, ?)";
$stmt = $conn->prepare($sql);
$stmt->bind_param("sisii", $student_username, $exam_id, $violation_type, $occurrence, $penalty);
